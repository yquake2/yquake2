 ...
 * =======================================================================
 *
 * The CTF gamemode with all support functions.
 *
 * =======================================================================
 */ 
...
/* Index for various CTF pics, this saves us from calling gi.imageindex 
   all the time and saves a few CPU cycles since we don't have to do a
   bunch of string compares all the time.  These are set in CTFPrecache()
...
/*
 * go to a ctf point, but NOT the two
 ...
 * flag carrier defense, etc. Note that 
 * bonuses are not cumaltive. You get one,
...
CTFFragBonuses(edict_t *targ, edict_t *inflictor, edict_t *attacker)
{
	...
	if (targ->client->pers.inventory[ITEM_INDEX(enemy_flag_item)])
	{
		 ...
		 * carrier field on the other team */
		for (i = 1; i <= maxclients->value; i++)
		{
			ent = g_edicts + i;
...
CTFPickup_Flag(edict_t *ent, edict_t *other)
{
	...
	if (ctf_team == other->client->resp.ctf_team)
	{
		if (!(ent->spawnflags & DROPPED_ITEM))
		{
			/* the flag is at home base. if the player has 
			 * the enemy flag, he's just won! */
			if (other->client->pers.inventory[ITEM_INDEX(enemy_flag_item)])
			{
				...
				gi.sound(ent,
						CHAN_RELIABLE + CHAN_NO_PHS_ADD + CHAN_VOICE,
...
static void
CTFDropFlagTouch(edict_t *ent, edict_t *other, cplane_t *plane,
		csurface_t *surf)
{
...
/* 
 * Called from PlayerDie, 
 * to drop the flag from 
 * a dying player 
 */
void
...
CTFResetGrapple(edict_t *self)
{
	if (self->owner->client->ctf_grapple)
	{
		...
		gi.sound(self->owner, CHAN_RELIABLE + CHAN_WEAPON,
				gi.soundindex(
						"weapons/grapple/grreset.wav"), volume, ATTN_NORM, 0);
...
void
CTFGrappleTouch(edict_t *self, edict_t *other, cplane_t *plane,
		csurface_t *surf)
{
	float volume = 1.0;
...
CTFGrapplePull(edict_t *self)
{
	...
	if (self->enemy)
	{
			...
			!CheckTeamDamage(self->enemy, self->owner))
		{
			...
			gi.sound(self, CHAN_WEAPON, gi.soundindex(
						"weapons/grapple/grhurt.wav"), volume, ATTN_NORM, 0);
		}
		...
		int damage, int speed, int effect)
{
	...
	grapple->s.modelindex = gi.modelindex(
	...
	tr = gi.trace(self->s.origin, NULL, NULL, grapple->s.origin,
			grapple, MASK_SHOT);
...
/* This array is in 'importance order', 
   ...
   important when reporting their 
   names. */
struct
{
	char *classname;
...
CTFSay_Team_Location(edict_t *who, char *buf)
{
	...
	while ((what = G_Find(what, FOFS(classname), hot->classname)) != NULL)
	{
		...
		/* if we are here, there is more than one, 
		   find out if hot is closer to red flag
		   or blue flag */
...
CTFReady(edict_t *ent)
{
	...
	if (!j && t1 && t2)
	{
		...
		ctfgame.countdown = false; gi.positioned_sound(world->s.origin,
				world, CHAN_AUTO | CHAN_RELIABLE, gi.soundindex("misc/talk1.wav"),
				1, ATTN_NONE, 0);
	}
}
...
CTFAdmin_SettingsApply(edict_t *ent, pmenuhnd_t *p)
{
	...
	if (settings->matchsetuplen != matchsetuptime->value)
	{
		...
		{
			/* in the middle of a match, change it on the fly */
			ctfgame.matchtime = (ctfgame.matchtime - matchsetuptime->value *
	...
	if (settings->matchstartlen != matchstarttime->value)
	{
		...
		{
			/* in the middle of a match, change it on the fly */
			ctfgame.matchtime = (ctfgame.matchtime -
...
CTFAdmin_UpdateSettings(edict_t *ent, pmenuhnd_t *setmenu)
{
	...
	PMenu_UpdateEntry(setmenu->entries + i,
			text,
			PMENU_ALIGN_LEFT,
...
CTFAdmin_Settings(edict_t *ent, pmenuhnd_t *p)
{
	...
	menu = PMenu_Open(ent, def_setmenu,
				-1, sizeof(def_setmenu) / sizeof(pmenu_t),
				settings);
	...
	*text = 0;
	...
	for (i = 1; i <= maxclients->value; i++)
	{
		...
		Com_sprintf( st, sizeof(st), "%3d %-16.16s %02d:%02d %4d %3d%s%s\n",
				i, e2->client->pers.netname, (level.framenum - e2->client->resp.enterframe) / 600,
